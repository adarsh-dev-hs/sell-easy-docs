---
title: Data layer
description: How the web app talks to the API, covering the fetch client, errors, the TanStack Query wrappers, per-service hooks, cache keys, invalidation and optimistic updates.
icon: Database
---
...
flowchart LR
  ...
  C --> F["request(): fetch + Bearer + envelope"]
  F --> API[("API")]
...
The value comes from `frontend/.env.local`. Because it's a `NEXT_PUBLIC_` variable, it's inlined when the app is built, so restart `next dev` or rebuild after you change it. The login page shows the host it's connected to.

### `request()`

Every call goes through one private function, `request(method, path, { query, body, raw })`:

1. **Builds the URL.** `query` values that are `undefined`, `null` or `""` are left out. Arrays are joined with commas, which matches the API's CSV filter format. Empty arrays are left out.
2. **Adds headers.** It always sends `Accept: application/json`. It sends `Content-Type: application/json` when there's a body, and `Authorization: Bearer <token>` when `useAuthStore` holds a token.
3. **Handles network errors.** If `fetch` throws, it throws `ApiError(0, "network_error", "Cannot reach the API at … Is the backend running?")`.
