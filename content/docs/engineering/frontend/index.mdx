...
Where "the API" lives is a build-time switch, `NEXT_PUBLIC_DATA_SOURCE`:

- **`api`** (default): requests go over HTTP to the Express backend at `NEXT_PUBLIC_API_URL`.
- **`mock`**: requests go to an in-browser copy of the backend (`src/mock-api`) that stores demo data in `localStorage`. No server is needed, so the frontend can be deployed alone (for example on Vercel) for demos. See [Mock data mode](/docs/engineering/frontend/mock-mode).

Pages and hooks don't know which one is active. Only `src/lib/api/client.ts` does.
...
Earlier prototypes kept a hand-written demo database in `localStorage` (`sell-easy-demo`). That's gone, and `SessionGate` deletes the old key on load. (Mock mode is different: it runs the **real** backend code, generated from `backend/src`, and uses its own `selleasy-mock-db:*` keys.) The rules now are:

- **All business logic lives in the API code.** That includes scoring, dedup, enrollment eligibility, agent runs, analytics and CSV export.
- **Only the session is persisted by the UI.** The `selleasy-session` key holds `{ token, user, org }`. `user` and `org` are a cache, refreshed from `GET /auth/me` on every load.
- **Every read goes through a hook in `@/lib/api`.** Pages never call `fetch` (or the mock API) themselves.
- **Mock mode is visible.** When `IS_MOCK` is true, the login page pre-fills the demo login and says "Demo mode · sample data stored in this browser", and the header shows a **Demo data** badge.
...
├── mock-api/                   In-browser API for NEXT_PUBLIC_DATA_SOURCE=mock
│   ├── server.ts               mockRequest(): route matching, demo seeding, latency, resetMockDatabase()
│   ├── core/                   GENERATED copy of backend/src (npm run mock:sync) — don't edit
│   ├── overrides/              Browser versions of config, logger, crypto, http, auth, json-repository
│   └── shims/                  express-rate-limit no-op
└── lib/
    ├── api/                    client.ts (api/mock switch), query.ts, auth-store.ts, index.ts, hooks/*
...
| `AppHeader` | `useAgentSettings`, `useSetAutopilot`, `useSimulateSignal`, `useAccount` and `useSequence` (for breadcrumb labels). Shows the **Demo data** badge when `IS_MOCK`. | `/agent/settings`, `POST /signals/simulate` |
...
flowchart TD
  ...
  Q --> A["api client<br/>(Bearer token)"]
  A -->|"api: fetch"| API[("SellEasy API")]
  A -->|"mock: mockRequest()"| MOCK[("src/mock-api<br/>+ localStorage")]
```
...
<Cards>
  ...
  <Card title="Mock data mode" href="/docs/engineering/frontend/mock-mode" description="Running the backend code in the browser for frontend-only demos." />
