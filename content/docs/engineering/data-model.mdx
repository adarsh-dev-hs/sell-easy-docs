...
- **Ids** come from `newId(prefix)` in `lib/ids.ts`: 6 random bytes in base64url, with `-` and `_` replaced by `x`.
...
<TypeTable
  type={{
    ...
    status: { type: '"active" | "invited"', description: 'Only active users can log in or pass authenticateJwt.', required: true },
...
<TypeTable
  type={{
    ...
    createdBy: { type: 'string', description: 'User id. Becomes req.auth.userId for API-key requests.', required: true },
