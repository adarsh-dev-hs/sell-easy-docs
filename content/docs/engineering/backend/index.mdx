...
| HTTP | Express | `^5.2.1` | `src/app.ts`. Async handler errors reach the error handler without wrappers. |
...
| `src/lib/http.ts` | `createModule()` / `route()`, shared query schemas, `paged()`, `HANDLED`. |
| `src/db/` | Storage abstraction and drivers. `db` is the registry of all collections. |
...
| `src/events/` | `EventBus` interface and the in-memory implementation. |
...
```mermaid
flowchart LR
  subgraph HTTP["HTTP layer — routes.ts"]
    r["route(def)<br/>zod schemas · roles · summary"]
  end
...
- **Routes** declare the contract: method, path, zod schemas, roles and a one-line summary. They pull typed values out of `ctx` and call a service. Simple reads and deletes sometimes use `db.*` directly in the route (for example `GET /signals/:id` and `DELETE /agent/playbooks/:id`).
...
- Route files export `xxxModule = createModule("<Tag>", "<basePath>")`. Register new modules in the `modules` array in `src/app.ts`.
...
<Cards>
  <Card title="Request lifecycle" href="/docs/engineering/backend/request-lifecycle" description="Middleware, route(), validation, errors and OpenAPI." />
  <Card title="Data layer" href="/docs/engineering/backend/data-layer" description="Repository interface, JSON driver internals, Postgres plan." />
