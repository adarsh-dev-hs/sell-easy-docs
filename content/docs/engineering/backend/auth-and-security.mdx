...
Authentication is local to the API (`AUTH_DRIVER=local`). Users get HS256 JWTs signed with `JWT_SECRET` and passwords are hashed with bcrypt. Machine clients (vendor webhooks, Zapier) use organization API keys, stored only as SHA-256 hashes. Authorization is a four-level role model enforced by the `route()` wrapper. Tenant isolation is enforced in the repository layer. Cognito, Auth0 and Google/Microsoft SSO exist only as env keys.

Relevant files: `src/middleware/auth.ts`, `src/modules/auth/routes.ts`, `src/modules/organization/service.ts`, `src/lib/crypto.ts`, `src/lib/http.ts`, `src/app.ts`.
...
`authenticateJwt(req)` runs for every route with `auth: "user"` (the default):

<Steps>
<Step>It requires `Authorization: Bearer <token>`. Otherwise it returns `401 Authentication required`.</Step>
...
<Step>It sets `req.auth = { userId, orgId, role: user.role, via: "jwt" }`. The **stored** role is used, not the claim, so role changes and removals apply on the next request.</Step>
</Steps>
...
| Secret format | `randomSecret("se_live_", 32)`: the prefix `se_live_` plus 32 characters from `[a-zA-Z0-9]` (`crypto.randomBytes`, mapped by modulo). |
...
| Lookup | `authenticateApiKey` reads the `x-api-key` header, or falls back to `Authorization: Bearer <key>`. It then calls `db.apiKeys.findGlobal({ hash: sha256(key), revoked: false })`. |
...
| Principal | `req.auth = { userId: record.createdBy, orgId: record.orgId, role: "member", via: "apiKey", scopes }` |
...
SHA-256 without a salt is acceptable here because the secrets are high-entropy random strings (about 190 bits), so they are not open to dictionary attacks the way passwords are.
...
- The tenant id comes only from the verified principal (`req.auth.orgId`). No route takes an `orgId` parameter.
...
- Every body, query and path parameter goes through zod in `route()`. Unknown body keys are **stripped** (zod's default for `z.object`), so clients can't set server-owned fields like `orgId`, `score` or `passwordHash`.
