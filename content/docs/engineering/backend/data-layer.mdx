---
title: Data layer
description: The Repository interface and query language, JsonRepository internals, the collection registry, and how to implement the Postgres driver.
icon: HardDrive
---

Services never touch storage directly. They call `db.<collection>`, which returns a `Repository<T>` (`src/db/repository.ts`). The driver behind it is chosen by `DB_DRIVER` in `src/db/index.ts`. Today the only working driver is `JsonRepository` (`src/db/json/json-repository.ts`), which keeps one JSON array file per collection in `DATA_DIR`.
...
### Operator semantics (JSON driver)
...
`sortRecords` sorts in place with the keys in order:
...
sequenceDiagram
  ...
  R->>M: load() — first call reads <name>.json once
  M-->>R: rows
  ...
  Q->>F: writeFile(<name>.json.<pid>.tmp)
  Q->>F: rename(tmp → <name>.json)
  Q-->>S: structuredClone(record)
```

| Mechanism | Implementation | Why |
|---|---|---|
| **Lazy load and cache** | `load()` reads `<dataDir>/<name>.json` once into `Map<id, record>`. A concurrent first read shares the same `loading` promise. A missing file (`ENOENT`) gives an empty collection, and an empty file counts as `[]`. | Reads never hit the disk after warm-up. |
| **Validation on load** | Content that isn't a JSON array throws `must contain a JSON array`. | Catches manual editing mistakes. |
| **Per-collection write queue** | `mutate(fn)` chains onto `this.queue`. Each step loads, runs `fn`, and persists. A failed step doesn't break the chain (`run.catch(() => undefined)`), but the error is still returned to the caller. | Concurrent requests can never interleave file writes. |
| **Atomic persistence** | `writeFile` to `<file>.<pid>.tmp` (pretty-printed, 2-space indent), then `rename` over the target. `mkdir -p` the data dir first. | A crash never leaves a half-written file. |
...
- **A corrupt file breaks the collection.** If the file fails to parse, the rejected `loading` promise is cached and every call fails until the process restarts.
...
- **One process only.** The cache is never refreshed from disk, and the temp file name includes the pid. Two API processes on the same `DATA_DIR` will overwrite each other's changes.
- **Every write rewrites the whole file.** Prefer `createMany` and `updateMany` to loops.
</Callout>

## Collection registry

`src/db/index.ts` creates every repository through `collection(name, idPrefix)`. `name` is both the file name and the collection or table name.
...
The files are plain, pretty-printed JSON arrays, so they are easy to inspect and patch. Before you edit:
