...
- **`frontend/`** is a Next.js 16 UI. It only displays and edits data through the API.
...
|---|---|---|
| Frontend framework | Next.js 16.3 (App Router, Turbopack), React 19.2, TypeScript 5 | Every page is a client component. The UI keeps no domain data in the browser. |
...
| Storage | JSON-file database (`backend/db/*.json`) behind a `Repository<T>` interface | The Postgres driver is a placeholder and not implemented yet |
...
| Events | In-memory `EventBus` (Node `EventEmitter`) | EventBridge/SQS are planned |
...
2. `lib/api/client.ts` sends the JSON request with the bearer token.
3. `createModule().route()` authenticates the caller, checks the role, validates the input with zod and calls the service.
...
| Path | What it is |
|---|---|
| `package.json`, `scripts/setup.mjs` | Root tooling. `install:all`, `setup`, `dev` (runs both apps with `concurrently`), `build`, `test` |
...
| `backend/src/lib/http.ts` | `createModule()` and `route()`: validation, auth, roles, the response envelope and OpenAPI registration |
...
| `frontend/src/lib/api/` | API client, query helpers, session store and hooks |
...
- **The backend owns all data and rules.** Scoring, deduplication, enrollment eligibility, agent orchestration, CRM sync and analytics all run on the server. The frontend only holds the session token and a cached copy of the user and org.
