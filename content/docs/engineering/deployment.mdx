...
| Web | `npm --prefix frontend run build` (`next build`, with `NEXT_PUBLIC_DATA_SOURCE` and `NEXT_PUBLIC_API_URL` set **at build time**) | `npm --prefix frontend start` → `next start` | 3000 (Next.js default, or `PORT`) |
...
Mock data mode lets you ship a clickable demo without hosting the API, a database or secrets. The browser runs the generated copy of the backend in `frontend/src/mock-api/core` and keeps the data in `localStorage`. See [Mock data mode](/docs/engineering/frontend/mock-mode).

<Steps>
<Step>

### Make sure the mock core is current

```bash
cd frontend
npm run mock:check    # must print "mock core up to date"
```

If it fails, run `npm run mock:sync` and commit the regenerated `src/mock-api/core`. Vercel builds only the frontend (which is its own git repository), so the generated copy has to be committed: the build never reads `backend/`.
...
- **Root directory:** the folder that contains `frontend/package.json`. That's the repository root when you import the standalone frontend repository, or `frontend` in a monorepo checkout.
- **Framework preset:** Next.js (build `next build`, the default install command)
...
`NEXT_PUBLIC_API_URL` isn't needed in mock mode.
...
Deploy, then open the site. The login page is pre-filled with `jordan@acmegrowth.com` / `demo1234` and says "Demo mode · sample data stored in this browser". Every visitor gets their own workspace in their own browser, seeded on first use.

</Step>
</Steps>

<Callout type="warn" title="A mock deployment is a demo, not a product">
Mock tokens are unsigned and the mock "crypto" is not real, so never put real customer data or real vendor keys into a mock deployment. Webhooks and API keys can't be used from outside the browser. Nothing is shared between visitors or devices.
</Callout>

### Switching the deployment to the real API

When a backend is hosted:

1. Deploy the API (see the checklist below) and add the Vercel domain(s) to its `CORS_ORIGINS`, for example `CORS_ORIGINS=https://selleasy-demo.vercel.app`. Preview deployments use different domains, so list those too if they should work.
2. In Vercel, set `NEXT_PUBLIC_DATA_SOURCE=api` (or delete it, since `api` is the default) and `NEXT_PUBLIC_API_URL=https://<api host>/api/v1`.
3. **Redeploy.** `NEXT_PUBLIC_*` values are inlined at build time, so editing the variables has no effect until the next build.
4. Sign in with a user from the API's database. Data entered in mock mode stays in each visitor's browser and isn't migrated.

To go back to the demo, set `NEXT_PUBLIC_DATA_SOURCE=mock` and redeploy. A common setup is two Vercel projects (or environments) from the same repository: one `mock` for demos and one `api` for the real app.
...
- [ ] `CORS_ORIGINS` = the exact web origin(s), including the Vercel domain if the web app is hosted there. Never `*`.
...
- [ ] `NEXT_PUBLIC_DATA_SOURCE=api` (or unset) and `NEXT_PUBLIC_API_URL` pointing at the production API, set **before** the build
- [ ] The header shows no **Demo data** badge, and the login page shows "Connected to …" rather than "Demo mode"
</Accordion>
</Accordions>
...
|---|---|---|---|
| Web | CloudFront + S3, or Vercel | `NEXT_PUBLIC_DATA_SOURCE`, `NEXT_PUBLIC_API_URL` | None. The app is a client-rendered Next.js app. Confirm static-export compatibility, or use Vercel or a Node server. |
...
Run the web app on the host with `npm --prefix frontend run dev`, using the default `NEXT_PUBLIC_DATA_SOURCE` (`api`) and `NEXT_PUBLIC_API_URL`.
...
- **Warnings worth alerting on**: `… adapter not implemented yet; using mock`, `Event handler failed`, `Slack notification failed`, and `No organization found in the database` at startup.
