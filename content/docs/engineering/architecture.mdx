...
flowchart TB
  ...
  subgraph "API process (Node ≥ 20)"
    ...
    routes["Module routers<br/>createModule().route()"]
    ...
    json["JsonRepository<br/>in-memory cache + write queue"]
...
|---|---|---|---|
| Web app | Next.js 16, React 19, shadcn/ui, TanStack Query 5, zustand | `frontend/` | Rendering, forms, client-side routing. Holds **no business data**. The only persisted browser state is the session (`selleasy-session`). |
...
|---|---|---|---|
| Web hosting | CloudFront + S3, or Vercel | `next dev` / `next start` locally | n/a |
...
| Event bus | EventBridge / SQS | `MemoryEventBus` (Node `EventEmitter`); **nothing subscribes** | `EVENT_BUS_DRIVER` (non-`memory` logs a warning and falls back) |
...
sequenceDiagram
  ...
  X->>R: module router match
  R->>R: authenticateJwt → re-read user → assertRole
  ...
  S-->>R: paged(page)
  R-->>C: 200 { data, meta: { total, page, pageSize } }
  C-->>Q: { data, meta }
  Q-->>B: render
```

A write follows the same path. The repository call goes through the per-collection write queue (`mutate`), which rewrites the whole file (`<name>.json.<pid>.tmp`, then `rename`). On success the frontend's `useApiMutation` invalidates **every** cached query by default, and the affected screens refetch.
...
- Every persisted record carries `orgId` (`Stored<T>` in `domain/types.ts`). Every `Repository` method except `findGlobal` and `truncate` takes `orgId` as its first argument, and `JsonRepository` filters on it (`scoped()`), including in `get`, `update` and `remove`.
- `orgId` always comes from the authenticated principal (`req.auth.orgId`, exposed to handlers as `ctx.orgId`). It is never read from the request body or URL.
...
**Decision.** Services depend only on `Repository<T>` (`db/repository.ts`). It has a deliberately small query language (`eq`, `ne`, `in`, `nin`, comparisons, `exists`, `contains`, `$or`) that maps directly to SQL. The first driver is `JsonRepository`: one file per collection, an in-memory cache, and atomic writes. `DB_DRIVER` selects the driver in `db/index.ts`.

**Consequences.** Zero-setup local development, and files you can inspect by hand. The driver is single-process only and keeps full collections in memory. Every write rewrites the whole file. There are no transactions: multi-collection operations such as cascading deletes are not atomic. The Postgres driver is a stub that throws.
</Accordion>
<Accordion title="ADR-2 · zod-driven routes and a generated OpenAPI document">
**Decision.** Every endpoint is declared with `createModule(tag, basePath).route({ method, path, body, query, params, roles, handler })`. The wrapper validates with zod, authenticates, checks roles, wraps the response and records the route in `routeRegistry`. `buildOpenApi()` turns the registry into OpenAPI 3.1 with `z.toJSONSchema`.

**Consequences.** Validation, docs and handler types can't drift apart. Response schemas are **not** described (the responses have empty `content`). The frontend keeps a hand-maintained copy of the types in `frontend/src/lib/types.ts`.
</Accordion>
...
**Decision.** Tokens are HS256 JWTs with `sub`, `org` and `role` claims. `authenticateJwt` still loads the user on every request and uses the stored role and status.
