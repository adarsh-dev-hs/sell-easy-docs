...
Non-catalog services used by the platform itself: **Slack** incoming webhook (live in production), **Sentry** and **Datadog** (planned), **Vercel** (demo hosting), **Stripe** or similar for billing (Phase 4, to be decided).
...
<Steps>
<Step>
**Claude drafting (Phase 1).** The most visible gap: the UI already says "Draft outreach with Claude". Platform-owned, no customer setup, easy to demo.
</Step>
<Step>
**One enrichment vendor + SES (Phase 1).** Pilot customers need real firmographics and must be able to send. SES is also needed for invite emails.
</Step>
<Step>
**RB2B webhook + HubSpot (Phase 2).** One real intent source proves the signal loop. HubSpot is the most common CRM among the target mid-market customers.
</Step>
<Step>
**Trigify, Bombora, second enrichment vendor (Phase 2).** Broadens signal coverage and completes the waterfall.
</Step>
<Step>
**Instantly / Mailpool, HeyReach, Apify, Linkup (Phase 3).** Volume sending, LinkedIn and research for GA.
</Step>
<Step>
**Salesforce, Attio, Fireflies, Limadata (Phase 4).** Enterprise CRM and call intelligence.
</Step>
</Steps>
