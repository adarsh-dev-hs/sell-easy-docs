...
Review this page every two weeks during Phases 1–2. Roadmap-wide risks (hiring, vendor contracts, later phases) are in [Roadmap → Dependencies & risks](/docs/roadmap/dependencies-and-risks).
...
| R5 | **Scheduler and inbound capture** are more complex than estimated (time zones, threading, bounces) | H | M | 🔴 6 | Start in Phase 2 week 1; narrow MVP to SES only; property-based tests for send windows; out-of-office handling as a Should | Backend engineer |
...
|---|---|---|---|
| A1 | Target customers use **HubSpot** as their CRM | Qualify design partners on CRM | Pull Salesforce forward; delay other Phase 4 work |
...
| A5 | SES is enough for pilot sending volumes (≤ ~20k emails per month in total) | SES limits and deliverability tests | Pull Instantly or Mailpool forward from Phase 3 |
...
| A7 | Existing product logic (scoring, agent, dedup) is correct at real data volumes | Staging soak with partner-sized data | Budget stabilisation time in Phase 2 |
| A8 | The team in [Team plan](/docs/team-and-cost/team-plan) is hired by the end of month 2 | Hiring pipeline tracking | Descope Should items; extend Phase 2 |
| A9 | 5–8 design partners can be signed before Phase 1 ends | Founder pipeline | Delay MVP launch; use internal dogfooding |
...
Any request to add scope to the MVP after Phase 1 starts must:
...
4. Be recorded in the release notes and in this page's change log.
...
| Date | Change | By |
|---|---|---|
| 2026-09-17 | Initial MVP risk register and assumptions | Product |
